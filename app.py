        ...
        if response.status_code == 200:
            ...
            return {
                ...
                "data": {
                    ...
                    "remaining_time": convert_seconds_to_human(countdown) if countdown > 0 else "N/A"
                }
            }
        else:
            return {
    ...
    <head>
        ...
        <style>
            :root {
                ...
                --accent: #ff4757; /* Professional Red */
                --accent-hover: #ff6b81;
                --border-color: #2a2a2e;
                --success: #2ed573;
            }
        ...
        <script>
            async function checkStatus() {
                ...
                try {
                    ...
                    // Small delay to make the animation feel better
                    ...
                    if (result.status === 'success') {
                        ...
                        // Color color based on state
                        if(result.state === 'Secured') {
                            stateEl.style.color = "#2ed573"; // Green
                        } else if (result.state === 'Unsecured') {
                            stateEl.style.color = "#ffa502"; // Orange
                        } else {
                            stateEl.style.color = "#1e90ff"; // Blue for pending
                        }

                        document.getElementById('resCurrent').innerText = result.data.current_email;
                        document.getElementById('resPending').innerText = result.data.pending_email;
                        document.getElementById('resTime').innerText = result.data.remaining_time;
                        document.getElementById('resMsg').innerText = result.message;
                    } else {
                        // Error handling in UI
                        stateEl.innerText = "Error";
                        stateEl.style.color = "#ff4757"; // Red
...
if __name__ == '__main__':
    # Using a different port just in case, feel free to change it back to 5002
